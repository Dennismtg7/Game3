...
(() => {
  ...
  // ---------- guest (local-only) storage keys ----------
  const GUEST_INDEX_KEY = "fieldbook:__index";
  const GUEST_CURRENT_KEY = "fieldbook:__current";
  const guestProjectKey = (name) => `fieldbook:project:${name}`;

  // ---------- state ----------
  let sheet = blankSheet();
  let currentProjectId = null; // guest: project name. signed in: Firestore doc id.
  let currentUid = null; // null => guest/local mode
  ...
  function sheetHasContent(s) {
    const metaFilled = ["date", "levelType", "surveyor", "booker", "fromPoint", "toPoint"].some((k) => (s[k] || "").trim() !== "");
    const rowsFilled = s.rows.some((r) => ["bs", "is", "fs", "distance", "remarks"].some((k) => (r[k] || "").trim() !== ""));
    return metaFilled || rowsFilled;
  }
  ...
  function renderSyncStatus() {
    if (currentUid) {
      ...
      syncStatusEl.textContent = "Working locally on this device";
      syncStatusEl.className = "sync-status";
    }
  }

  // ---------- guest (local) persistence ----------
  function guestGetIndex() {
    try {
  ...
  function guestSetIndex(list) {
    localStorage.setItem(GUEST_INDEX_KEY, JSON.stringify(list));
  }
  function guestEnsureIndexed(name) {
    const list = guestGetIndex();
    if (!list.includes(name)) {
      list.push(name);
      guestSetIndex(list);
    }
  }
  function refreshGuestSavedList() {
    savedList = guestGetIndex().map((name) => ({ id: name, name }));
  }
  function guestLoadCurrent() {
    try {
      const raw = localStorage.getItem(GUEST_CURRENT_KEY);
  ...
  // ---------- firestore paths (signed-in mode) ----------
  ...
  function persistCurrent() {
    if (currentUid) {
      ...
      localStorage.setItem(GUEST_CURRENT_KEY, JSON.stringify(sheet));
      if (currentProjectId) {
        localStorage.setItem(guestProjectKey(currentProjectId), JSON.stringify(sheet));
      }
    }
  }
  ...
  document.getElementById("btn-save").addEventListener("click", async () => {
    ...
    try {
      if (currentUid) {
        ...
        localStorage.setItem(guestProjectKey(name), JSON.stringify(sheet));
        guestEnsureIndexed(name);
        currentProjectId = name;
        refreshGuestSavedList();
      }
      persistCurrent();
      renderActiveProject();
    ...
    if (delBtn) {
      ...
      try {
        if (currentUid) {
          ...
          localStorage.removeItem(guestProjectKey(id));
          guestSetIndex(guestGetIndex().filter((n) => n !== id));
          refreshGuestSavedList();
          renderLibraryList();
        }
      } catch (err) {
    ...
    if (item) {
      ...
      try {
        if (currentUid) {
          ...
          const raw = localStorage.getItem(guestProjectKey(id));
          if (raw) {
  ...
  const DEFAULT_BLURB = "Sign in to back up this field book to the cloud and access it from other devices. What you've entered so far stays right where it is either way.";
  ...
  function setAuthMode(mode) {
    ...
    if (mode === "signin") {
      ...
      authBlurb.textContent = DEFAULT_BLURB;
  ...
  btnSignin.addEventListener("click", openAuthOverlay);
  btnCloseAuth.addEventListener("click", () => (authOverlay.hidden = true));
  ...
  // ---------- boot: start in guest mode immediately, no waiting on auth ----------
  const guestCurrent = guestLoadCurrent();
  if (guestCurrent) sheet = guestCurrent;
  refreshGuestSavedList();
  renderMeta();
  renderRows();
  renderActiveProject();
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (!user) {
      // guest / signed-out mode
      currentUid = null;
      currentProjectId = null;
      const local = guestLoadCurrent();
      sheet = local || blankSheet();
      refreshGuestSavedList();
    ...
    // live list of this account's saved field books (also primes the offline cache)
    ...
    // decide whether to keep the guest sheet just entered, or pull the account's cloud draft
    try {
      if (sheetHasContent(sheet)) {
        // preserve what they were just working on locally, adopt it as their cloud draft
        currentProjectId = null;
        persistCurrent();
      } else {
