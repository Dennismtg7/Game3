...
(() => {
    ...
    try {
      if (authMode === "signin") {
        ...
        if (!cred.user.emailVerified) {
          ...
          await signOut(auth);
          showAuthError("Please verify your email before signing in — check your inbox (and spam folder) for the link.");
        ...
        showAuthNotice(`Verification email sent to ${email}. Confirm it, then sign in here.`);
      }
    } catch (e) {
  ...
  document.getElementById("btn-forgot").addEventListener("click", async () => {
    ...
    try {
      ...
      showAuthNotice("Password reset email sent.");
    } catch (e) {
      showAuthError(friendlyAuthError(e));
    }
  });
